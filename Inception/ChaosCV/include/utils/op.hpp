...
namespace chaos
{
	CHAOS_API Tensor operator+(const Tensor& a, const Tensor& b); // broadcast add
	CHAOS_API Tensor operator+(float a, const Tensor& b);
	CHAOS_API Tensor operator+(const Tensor& a, float b);

	CHAOS_API Tensor operator-(const Tensor& a, const Tensor& b); // broadcast sub
	CHAOS_API Tensor operator-(float a, const Tensor& b);
	CHAOS_API Tensor operator-(const Tensor& a, float b);
	...
	CHAOS_API void div(const Tensor& a, const Tensor& b, Tensor& c); // broadcast div
	CHAOS_API void gemm(bool transA, bool transB, const Tensor& a, const Tensor& b, float alpha, Tensor& c, float beta);
	CHAOS_API void invert(const Tensor& a, Tensor& b, int method = 1);
