...
namespace chaos
{
	...
	{
	public:
		VulkanWindow(const std::wstring& name, uint32 width, uint32 height, const VulkanDevice* vkdev) : vkdev(vkdev)
		{
			...
			present_format = vkdev->GetSurfaceFormat(surface);
		...
		VkFramebuffer* CreateFrameBuffer(const VkRenderPass& render_pass) final
		{
		...
		int height() const final { return extent.height; }
		int width() const final { return extent.width; }
		int format() const final { return present_format.format; }
	...
	Ptr<Window> Window::Create(const std::wstring& name, uint32 width, uint32 height, const VulkanDevice* vkdev)
	{
		return Ptr<Window>(new VulkanWindow(name, width, height, vkdev));
	}
