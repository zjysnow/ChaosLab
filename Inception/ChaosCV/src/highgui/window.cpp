...
namespace chaos
{
	...
	{
	public:
		VulkanWindowImpl(const std::wstring& name, const VulkanDevice* vkdev, uint32 width, uint32 height) : vkdev(vkdev), pen(vkdev)
		{
			auto h_instance = GetModuleHandle(NULL);
			WNDCLASS window_class{};
			window_class.hbrBackground = (HBRUSH)(COLOR_WINDOW + 1);
		...
		void Show() final
		{
			...
			while (GetMessage(&msg, NULL, 0, 0))
			{
				TranslateMessage(&msg);
				DispatchMessage(&msg);
				cmd.Present(swap_chain, present_queue_family_index, std::bind(&VulkanWindowImpl::RecreateSwapChain, this), std::bind(&VulkanWindowImpl::UpdateUniform, this, std::placeholders::_1));
			}
			vkDeviceWaitIdle(vkdev->GetDevice());
		}
		...
		void CreateFrameBuffers()
		{
			...
			for (size_t i = 0; i < image_count; i++)
			{
				...
				VkFramebufferCreateInfo framebuffer_info{};
				framebuffer_info.sType = VK_STRUCTURE_TYPE_FRAMEBUFFER_CREATE_INFO;
				framebuffer_info.renderPass = pen.pipeline->render_pass;
				framebuffer_info.attachmentCount = 1;
				framebuffer_info.pAttachments = attachments;
				framebuffer_info.width = extent.width;
				framebuffer_info.height = extent.height;
				...
				VkResult ret = vkCreateFramebuffer(vkdev->GetDevice(), &framebuffer_info, nullptr, &swap_chain_frame_buffers[i]);
				CHECK_EQ(VK_SUCCESS, ret) << "vkCreateFramebuffer failed " << ret;
			}
		}
		...
		void UpdateUniform(uint32_t image_index)
		{
