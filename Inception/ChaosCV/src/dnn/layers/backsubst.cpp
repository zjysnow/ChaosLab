...
namespace chaos
{
	inline namespace dnn
	{
        ...
        {
            auto axpy = [](int m, int n, const auto* x, int dx, const auto* a, int inca, auto* y, int dy) {
            ...
            for (i = 0; i < nm; i++, u += udelta0, v += vdelta0)
            {
                ...
                else
                {
                    if (b)
                    {
                        for (j = 0; j < nb; j++)
                            buffer[j] = 0;
                        axpy(m, nb, b, ldb, u, udelta1, buffer, 0);
                    ...
                    axpy(n, nb, buffer, 0, v, vdelta1, x, ldx);
                }
            }
        }
